...
contract GrantTokenRWAFactoryAdminToUser is Script {
    function run() external {
        ...
        address tokenRWAFactoryAddress = vm.envAddress(
            "TOKEN_RWA_FACTORY_ADDRESS"
        );
        ...
        TokenRWAFactory tokenRWAFactory = TokenRWAFactory(
            tokenRWAFactoryAddress
        );

        // Check if user already has ADMIN_ROLE
        bool hasAdminRole = tokenRWAFactory.hasRole(
            tokenRWAFactory.ADMIN_ROLE(),
            userAddress
        );

        if (!hasAdminRole) {
            // Grant ADMIN_ROLE to the user
            tokenRWAFactory.grantRole(
                tokenRWAFactory.ADMIN_ROLE(),
                userAddress
            );
