...
export const ClaimHistory = () => {
    ...
    if (isLoading) {
        return (
            <div className="p-6 rounded-xl border border-gray-100 bg-white/50 backdrop-blur-sm shadow-sm flex justify-center items-center h-40">
    ...
    if (claims.length === 0) {
        return (
            <div className="p-8 rounded-xl border border-gray-100 bg-white/50 backdrop-blur-sm shadow-sm">
    ...
    return (
        <div className="space-y-6 max-w-6xl mx-auto">
            <div className="p-8 rounded-xl border border-gray-100 bg-white/50 backdrop-blur-sm shadow-sm">
                <h2 className="text-2xl font-semibold mb-6 text-gray-800">Claim History</h2>

                <div className="space-y-6">
                    {claims.map(claim => (
                        <div
                            key={claim.id}
                            className="p-6 rounded-xl border border-gray-100 bg-white shadow-sm hover:shadow-md transition-shadow duration-200"
                        >
                            <div className="flex justify-between items-start mb-4">
                                <div>
                                    <h3 className="text-lg font-semibold text-gray-800">{claim.tokenName}</h3>
                                    <div className="mt-1 flex items-center gap-3">
                                        <p className="text-sm text-gray-500">Claim ID: {claim.id}</p>
                                        <span className="h-1 w-1 rounded-full bg-gray-300"></span>
                                        <p className="text-sm text-gray-500">Policy ID: {claim.policyId}</p>
                                    </div>
                                </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                                <div className="space-y-4">
                                    <div className="bg-gray-50/50 p-4 rounded-lg">
                                        <h4 className="text-sm font-medium text-gray-600 mb-1">Claim Amount</h4>
                                        <p className="text-lg font-semibold text-gray-900">${claim.claimAmount.toLocaleString()}</p>
                                    </div>
                                    <div className="bg-gray-50/50 p-4 rounded-lg">
                                        <h4 className="text-sm font-medium text-gray-600 mb-1">Claim Date</h4>
                                        <p className="text-lg font-semibold text-gray-900">{formatDate(claim.claimDate)}</p>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-4">
                                    {claim.payoutAmount && (
                                        <div className="bg-gray-50/50 p-4 rounded-lg">
                                            <h4 className="text-sm font-medium text-gray-600 mb-1">Payout Amount</h4>
                                            <p className="text-lg font-semibold text-gray-900">${claim.payoutAmount.toLocaleString()}</p>
                                        </div>
                                    )}
                                    {claim.payoutDate && (
                                        <div className="bg-gray-50/50 p-4 rounded-lg">
                                            <h4 className="text-sm font-medium text-gray-600 mb-1">Payout Date</h4>
                                            <p className="text-lg font-semibold text-gray-900">{formatDate(claim.payoutDate)}</p>
                                        </div>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="mt-6 p-4 bg-gray-50/50 rounded-lg">
                                <h4 className="text-sm font-medium text-gray-600 mb-2">Reason for Claim</h4>
